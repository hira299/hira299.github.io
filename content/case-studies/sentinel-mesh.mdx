...
2. **Initial verification.** A Z3-based verifier checks the configuration against the Cloud Perimeter Model (CPM): invariants for network isolation, encryption at rest and in transit, and defense in depth. A configuration that passes needs no patch.
...
4. **Patch verification.** The HCL is extracted from the model response, re-parsed, and checked by a dual-solver Z3 patch proof.
...
Security properties are encoded as Z3 constraints over the parsed configuration. When a configuration violates an invariant, the solver finds a satisfying assignment for the violation, a counterexample witness. The verifier's verdict, built from that witness, is fed into the next prompt.

For candidate patches, verification uses a dual-solver refinement check. Solver A checks completeness: that no counterexample exists, so the patched configuration satisfies the full set of modeled invariants. Solver B checks non-regression: it uses the original configuration to confirm the violation was real, then shows that the patched configuration cannot retain that violation class. For high-risk policies, passing both produces a formal proof certificate.
...
- **Certificates cover a subset.** Formal proof certificates were issued for 29 of 105 cases, not for every accepted patch.
